...
function TeaItem({ tea, index }: { tea: Tea; index: number }) {
  ...
  return (
    <div
      ...
      className={`bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 scroll-animate ${itemAnimation.isVisible ? 'visible' : ''}`}
      style={{ animationDelay: `${index * 100}ms` }}
    >
      <div className="h-80 bg-gradient-to-br from-green-100 to-amber-100 flex items-center justify-center overflow-hidden relative">
        <Image
          src={tea.image}
...
export default function TeaCatalogPage() {
  const { t } = useTranslation()
  const heroAnimation = useScrollAnimation({ threshold: 0.5, rootMargin: '-100px 0px' })
  const gridAnimation = useScrollAnimation({ threshold: 0.2, rootMargin: '-50px 0px' })

  return (
    <div className="min-h-screen bg-gray-50">
      {/* Hero Section */}
      <section className="relative py-20 bg-white overflow-hidden -mt-20 pt-32">
        {/* Decorative leaf backgrounds */}
        <div className='absolute top-16 right-10 w-52 h-52 pointer-events-none opacity-20'>
        ...
        <div className='absolute top-60 left-8 w-40 h-40 pointer-events-none opacity-25'>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div 
            ...
            className={`text-center scroll-animate ${heroAnimation.isVisible ? 'visible' : ''}`}
          >
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-green-900 mb-6 font-serif">
      ...
      <section className="py-20 bg-gray-50 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div 
            ...
            className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 scroll-animate ${gridAnimation.isVisible ? 'visible' : ''}`}
          >
